...
def azimuthalAverage(image, center=None, stddev=False, returnradii=False, return_nr=False, 
    ...
    if stddev:
        ...
        radial_prof = np.array([np.nanmax(np.append((image*weights).flat[whichbin==b],-np.inf)) for b in range(1,nbins+1)])
    else:
        radial_prof = np.array([np.nansum((image*weights).flat[whichbin==b]) / weights.flat[whichbin==b].sum() for b in range(1,nbins+1)])

    sum_not_nan = np.array([np.sum(~np.isnan(image.flat[whichbin==b])) for b in range(1,nbins+1)])
    ww_all_nan = sum_not_nan == 0
    radial_prof[ww_all_nan] = np.nan
    # ww_not_all_nan = np.where(sum_not_nan > 0)[0]
    # radial_prof[np.max(ww_not_all_nan)] = np.nan
